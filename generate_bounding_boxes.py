                ...
                if not os.path.isfile(game_dir_full_path):
                    ...
                    for player in data["annotations"]:
                        ...
                        if (
                            ...
                            if not (
                                (nosex == 0 and nosey == 0)
                                or (neckx == 0 and necky == 0)
                            ):
                                halfFaceDistance = math.sqrt(
                                    (nosex - neckx) * (nosex - neckx)
                                ...
                                maxx = np.minimum(
                                    float(imageWidth), maxx + 2 * halfFaceDistance
                                )
                                miny = np.maximum(0.0, miny - 2 * halfFaceDistance)
                                maxy = np.minimum(
                                    float(imageHeight), maxy + 2 * halfFaceDistance
                                )
                            else:
                                ...
                                maxy = np.minimum(
                                    float(imageHeight), maxy + height * 0.1
                                )
                            ...
                            if len(outofrangekeypoints) > 0:
                                ...
                                if (
                                    len(
                                        list(
                                            set(upperPoints) & set(outofrangekeypoints)
                                        )
                                    )
                                    > 0
                                    and miny < 30
                                ...
                                if (
                                    len(
                                        list(set(leftPoints) & set(outofrangekeypoints))
                                    )
                                    > 0
                                    and minx < 30
                                ):
                                ...
                                if (
                                    len(
                                        list(
                                            set(rightPoints) & set(outofrangekeypoints)
                                        )
                                    )
                                    > 0
                                    and imageWidth - maxx < 30
                                ):
                                    maxx = imageWidth
                    ...
                    outputFilenameString = (
                        f"{game_dir_full_path}/{game_dir_name}-bbox-appended.json"
                    )
                    ...
                    outputFilenameString = (
                        f"{game_dir_full_path}/{game_dir_name}-bbox-only.json"
                    )
                    outputFile = open(outputFilenameString, "w")
