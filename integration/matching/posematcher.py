...
class PoseMatcher:
    ...
    def match(self):
        ...
        for t in tqdm(td_estimations):
            ...
            td_pts = format_keypoints(t["keypoints"], 1, 1)
            # match
            bu_pred = bu_estimations[img_id]
            bu_pred = np.array([
                format_keypoints(pred["keypoints"], 1, 1) for pred in bu_pred
            ])
            p_aligned, _ = self._best_match(td_pts, bu_pred)
            p_aligned = np.float32(p_aligned)

            #Normalize p_aligned
            p_aligned[0, :] = p_aligned[0, :] / 640
            p_aligned[1, :] = p_aligned[1, :] / 360
